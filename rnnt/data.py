...
class BaseData:
    ...
    def _get_padded_aud(
        ...
        max_len = 1 + math.ceil(
            (max_duration -320)/160
        )
        folder = 'files/MFCC/'
        new_path = folder + aud_path[5:-4] + "/" + word[1:-1] + ".npy"
    ...
    def prepare_audio(self, audio_path: Union[str, Path]) -> Tensor:
        ...
        x = MFCC(
            n_mfcc=30,
            melkwargs={"n_fft": 320, "hop_length": 160, "n_mels": 40, "center": False},
        )(x)
        x = x.permute(0, 2, 1)
        return x
...
class TrainDataLoader(BaseData):
    def __init__(
        self,
    ...
    def get_max_duration(self, start_idx: int, end_idx: int) -> float:
        durations = self.df.loc[start_idx:end_idx, 'end'] - self.df.loc[start_idx:end_idx, 'start']
    ...
    def get_audios(self, start_idx: int, end_idx: int) -> Tensor:
        ...
        results = [
            ...
            for path,word in zip(self.df['audio_path'].iloc[start_idx:end_idx], self.df['text'].iloc[start_idx:end_idx])
        ]
...
class BaseData1:
    ...
    def _get_padded_aud(
        ...
        max_len = 1 + math.ceil(
            (max_duration * 16000 -320)/160
        )

        folder = 'files/MFCC/'
        new_path = folder + aud_path[5:]
        new_path = new_path[:-4] + ".npy"
        aud = torch.tensor(np.load(new_path))
...
class TestDataLoader(BaseData1):
    def __init__(
        self,
        file_path: Union[str, Path],
        tokenizer,
...
if __name__=='__main__':
    import numpy as np
    import pandas as pd
    csv_path = 'files/test_final.csv'
    folder = 'files/MFCC'
    csv_file = pd.read_csv(csv_path)
    import os

    def prepare_audio(audio_path: Union[str, Path], start, end) -> Tensor:
        x, sr = torchaudio.load(audio_path, normalize=True)
        x = Resample(sr, 16000)(x)[:, start:end]
    ...
    for idx, row in csv_file.iterrows():
        ...
        new_audio_path = folder + "/" + audio_path.split('.')[0][5:] + '/' + text[1:-1]
        directory_path = "/".join(new_audio_path.split('/')[:-1])
        mfcc = prepare_audio(audio_path, start, end)
        os.makedirs(directory_path, exist_ok=True)
        np.save(new_audio_path, mfcc)
