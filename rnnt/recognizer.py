...
class KeywordRecognizer(QWidget):
    ...
    def initUI(self):
        ...
        layout.setContentsMargins(10, 10, 10, 10)  # Margins around layout
    ...
    def addAudioFiles(self):
        audio_paths = 'files/train_sentences.csv'
        paths_df = pd.read_csv(audio_paths)
        paths_df['audio_path'] = paths_df['audio_path'].str.replace('TRAIN', 'TEST')
        self.audioList.addItems(paths_df['audio_path'])

    def load_model(self):
        config_path = 'config/aishell.yaml'
        model_path = 'timit/rnnt/rnnt.epoch120.chkpt'
    ...
    def play_wav_file(self):
        selectedAudio = self.audioList.currentItem().text().replace('TEST', 'TRAIN')
